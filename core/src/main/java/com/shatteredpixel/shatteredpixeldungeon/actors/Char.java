	...
	public void damage( int dmg, Object src ) {
		...
		if (!(src instanceof Char)) {
			if (Dungeon.hero.alignment == alignment
				...
				rRD *=  0.925f - 0.075f*Dungeon.hero.pointsInTalent(Talent.AURA_OF_PROTECTION);
			}
		}
		...
		if (buff(PowerOfMany.PowerBuff.class) != null){
			if (buff(LifeLinkSpell.LifeLinkSpellBuff.class) != null){
				...
				rRD *= 0.70f - 0.05f*Dungeon.hero.pointsInTalent(Talent.LIFE_LINK);
			} else {
				damage *= 0.75f;
				rRD *= 0.75f;
			}
		...
		if (AntiMagic.RESISTS.contains(src.getClass())){
			float temp1 = dmg -= AntiMagic.drRoll(this, glyphLevel(AntiMagic.class));
			dmg -= temp1;
			rRD -= temp1;
			if (buff(ArcaneArmor.class) != null) {
				temp1 = Random.NormalIntRange(0, buff(ArcaneArmor.class).level());
				dmg -= temp1;
				rRD -= temp1;
			}
			if (dmg < 0) {
