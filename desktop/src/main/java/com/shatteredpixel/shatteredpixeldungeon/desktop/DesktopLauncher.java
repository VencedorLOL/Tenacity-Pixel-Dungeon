...
import static com.shatteredpixel.shatteredpixeldungeon.SPDSettings.getRoute;
...
public class DesktopLauncher extends Thread {
	...
	public static void main (String[] args) {
		...
		boolean usesShattered = getRoute();
		System.out.println(usesShattered);
		...
		Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
			...
			public void uncaughtException(Thread thread, Throwable throwable) {
				...
				if (exceptionMsg.contains("Couldn’t create window")) {
					System.out.println(title + " Has Crashed!"+
		...
		if (SharedLibraryLoader.isWindows) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				if (usesShattered)
					basePath = "Application Data/." + "shattered" + "/" + "Shattered Pixel Dungeon" + "/";
				else
					basePath = "Application Data/." + vendor + "/" + title + "/";
			} else {
				if (usesShattered)
					basePath = "AppData/Roaming/." + "shattered" + "/" + "Shattered Pixel Dungeon" + "/";
				else
			...
			if (usesShattered)
				basePath = XDGHome + "/." + "shattered" + "/" + titleLinux + "/";
			else
		...
		System.out.println("route is: " + basePath);
		try {
			new Lwjgl3Application(new ShatteredPixelDungeon(new DesktopPlatformSupport()), config);
		} catch (RuntimeException r) {
			if (Objects.equals(r.getMessage(), "ReOpenException")) {
				DesktopLauncher d = new DesktopLauncher();
				d.start();
			}
				// -------------------------------------------------------------------------------------------------------------
				// FOR SWITCHING ROOTS SUPPORT.
	...
	@Override
	public void run() {
		while (TitleScene.immediatelyReOpenTheGameAfterClosed) {
			TitleScene.immediatelyReOpenTheGameAfterClosed = false;
			// runs fkn everything again cuz i dont want to risk it doing it wrong

			SPDSettings.useShatteredRoute(getRoute());
			boolean usesShattered = getRoute();
			System.out.println(usesShattered);
